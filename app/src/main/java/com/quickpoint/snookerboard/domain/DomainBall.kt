...
fun MutableList<DomainBall>.isAddRedAvailable() = isThisBallColorAndNotLast() && !FrameToggles.FRAMETOGGLES.isFreeball
...
fun MutableList<DomainBall>?.availablePoints(): Int {
    ...
    val freeSize = (if (FrameToggles.FRAMETOGGLES.isFreeball) size - 1 else size)
    return if (freeSize <= 7) (-(8 - freeSize) * ((8 - freeSize) + 1) + 56) / 2 + (if (FrameToggles.FRAMETOGGLES.isFreeball) (9 - freeSize) else 0)
...
fun MutableList<DomainBall>.onPot(potType: PotType, potAction: PotAction) {
    when (potType) {
        ...
        TYPE_FREE_ACTIVE -> if (FrameToggles.FRAMETOGGLES.isFreeball) addFreeBall(1) else removeFreeBall()
...
fun MutableList<DomainBall>.onUndo(potType: PotType, potAction: PotAction, frameStack: MutableList<DomainBreak>) {
    when (potType) {
        ...
        TYPE_FREE_ACTIVE -> if (FrameToggles.FRAMETOGGLES.isFreeball) removeFreeBall() else addFreeBall(1)
