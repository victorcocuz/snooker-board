...
import com.quickpoint.snookerboard.utils.K_BOOL_TOGGLE_LONG_SHOT
import com.quickpoint.snookerboard.utils.K_BOOL_TOGGLE_REST_SHOT

@Composable
fun ModuleGameActions(mainVm: MainViewModel,gameVm: GameViewModel, ballsList: List<DomainBall>, frameStack: List<DomainBreak>) = ContainerColumn(Modifier.background(BrownDark)) {
    val ballSize = 48.dp

    GameButtonsIcons(mainVm, gameVm)
    StandardRow {
...
@Composable
fun GameButtonsIcons(mainVm: MainViewModel, gameVm: GameViewModel) {
    ...
    StandardRow(
        ...
        ) { mainVm.savePref(K_BOOL_TOGGLE_LONG_SHOT, !isLongActive) }
        if (isAdvancedStatistics) IconButton(
        ...
        ) { mainVm.savePref(K_BOOL_TOGGLE_REST_SHOT, !isRestActive) }
    }
}
